//
//  FavoritesFavedCollectionView.swift
//  Elated
//
...
class FavoritesFavedCollectionView: UICollectionView {

    private let disposeBag = DisposeBag()
    
    var data = BehaviorRelay<[Match]>(value: [])
    var didSendFavorites = PublishRelay<Match>()
    var didSendSpark = PublishRelay<Match>()
    let didSelectProfile = PublishRelay<Match>()
    var getMoreFans = PublishRelay<Void>()
    ...
    init() {
        ...
        self.register(FavoritesFavedCollectionCell.self,
                      forCellWithReuseIdentifier: FavoritesFavedCollectionCell.identifier)
...
extension FavoritesFavedCollectionView: UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return data.value.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: FavoritesFavedCollectionCell.identifier,
                                                      for: indexPath) as? FavoritesFavedCollectionCell ?? FavoritesFavedCollectionCell()
        let match = data.value[indexPath.item]
        cell.set(match)
        
        cell.favoriteAction = { [weak self] in
            self?.didSendFavorites.accept(match)
        }
...
extension FavoritesFavedCollectionView: UICollectionViewDelegateFlowLayout, UICollectionViewDelegate {
