...
class EditProfileOccupationViewController: ScrollViewController {
    ...
    private let label: UILabel = {
        let label = UILabel()
        label.text = "profile.editProfile.occupationTitle".localized
    ...
    private let textField = UITextField.createNormalTextField("profile.editProfile.occupation.placeholder",
                                                              font: .futuraBook(14),
                                                              cornerRadius: 25)
    ...
    init(_ type: EditInfoControllerType, occupation: String) {
    ...
    override func bind() {
        ...
        textField.rx.text.orEmpty.bind(to: viewModel.occupation).disposed(by: disposeBag)
        
        saveButton.rx.tap.bind { [weak self] in
            self?.viewModel.sendRequest(.occupation)
        }.disposed(by: disposeBag)
        ...
        viewModel.occupation.subscribe(onNext: { [weak self] arg in
